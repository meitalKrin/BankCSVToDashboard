...
| **R3** | 512 MB is not enough for Actual + bridge + Tailscale + Docker | High — forces different hardware | **Spike 0.3**: measure real resident memory under load; mitigations are batching, memory limits, zram; fallback is a Pi 4 or moving Actual off-box |
